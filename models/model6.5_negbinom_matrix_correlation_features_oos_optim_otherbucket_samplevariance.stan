...
transformed parameters {
    ...
    for (s in 1:S2) {
        corrected_phis2[s] = log_global_phi_scale + log_gene_phi + log_sample2_scale[s] + log(celltype_scale * sample2_x[s]);
...
model {
    ...
    for (s in 1:S) {
        sample_y[s] ~ neg_binomial_2_log(log_expected_rate[s], corrected_phis[s]);
    }
    ...
    for (s in 1:S2) {
        ...
        sample2_y[s] ~ neg_binomial_2_log(log_expected_rate2[s], corrected_phis2[s]);
    }
}
generated quantities {
    ...
    for (n in 1:N) {
        ...
        y_rep[n] = neg_binomial_2_log_rng(log_expected_rate_rep, corrected_phis[sample[n]][gene[n]]);
        log_lik[n] = neg_binomial_2_log_lpmf(y[n] | log_expected_rate_rep, corrected_phis[sample[n]][gene[n]]);
    }
}